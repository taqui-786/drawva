...
- NEW CREATION ANCHOR: Place your answer STRICTLY DIRECTLY BELOW the user's drawn input (changedBox), NEVER to its right, left, or above it. Center x under the drawing (x = changedBox.x + (changedBox.w - w) / 2) and sit the widget below with a clean 20px ink gap: y = changedBox.y + changedBox.h + 20. The gap sits cleanly below without touching drawn strokes.
- SCALE & ASPECT MATCHING: Match output size to the user's drawing footprint, maintaining a clear, readable aspect ratio (~1.4:1 to 1.6:1, e.g., w ~ 540..680, h ~ 360..460). Avoid squished flat strips (do NOT use paper-thin height) and avoid giant viewport-half boxes. Formulas (draw_formula) and text (write_text) MUST use generous font sizes (fontSize >= 140).
